...
namespace PianificazioneFrm.Gantt
{
    public partial class GanttChart : UserControl
    {
        ...
        int barStartLeft = 130;// GB inizialmente era 100
        ...
        public void AddChartBar(string rowText, string label, object barValue, DateTime fromTime, DateTime toTime, Color color, Color hoverColor, int rowIndex)
        {
            ChartBarDate bar = new ChartBarDate();
            bar.Text = rowText;
            bar.Value = barValue;
        ...
        public void AddChartBar(string rowText, string label, object barValue, DateTime fromTime, DateTime toTime, Color color, Color hoverColor, int rowIndex, bool hideFromMouseMove)
        {
            ChartBarDate bar = new ChartBarDate();
            bar.Text = rowText;
            bar.Label = label;
            ...
            foreach (ChartBarDate bar in bars)
            {
                ...
                if (bar.StartValue != null & endValue != null)
                {
                    ...
                    if ((index >= scrollPos & index < barsViewable + scrollPos) | ignoreScrollAndMousePosition == true)
                    {
                        ...
                        grfx.DrawString(bar.Text, rowTextFont, Brushes.Black, obRect.X, barStartTop + (barHeight * (index - scrollPos)) + (barSpace * (index - scrollPos)));

                        // Draws the rowtext

                        grfx.DrawString(bar.Label, rowTextFont, Brushes.Black, 0, barStartTop + (barHeight * (index - scrollPos)) + (barSpace * (index - scrollPos)));
